...
3. GET - / : /{part} url로 이동할 수 있는 메인 페이지를 출력
4. PUT - /disease/{part} : name, tag를 body로 넘겨 받아 해당 part DB에 추가 또는 수정
5. DELETE - /disease/{disease}/{part} : 해당 part DB에서 disease라는 이름을 갖는 질병 삭제
6. GET - /part/{part} : 해당 part DB에 있는 모든 질병에 대한 정보를 json 형태로 return
```